...
from project_paths import artifact_data_file, model_file, config_file, figure_file

# === Load Everything ===
print("🔍 Loading embeddings and labels...")
embeddings = np.load(artifact_data_file("node2vec_embeddings.npy"))
y_true = np.load(artifact_data_file("y_labels.npy"))
le = joblib.load(model_file("label_encoder.pkl"))
...
clf_stage1 = joblib.load(model_file("stage1_EN_vs_NONEN.pkl"))
clf_stage2 = joblib.load(model_file("stage2_NONEN_groups.pkl"))

# === Load Original One-Stage Model
print("🛠️ Loading one-stage XGBoost model...")
clf_one_stage = joblib.load(model_file("xgboost_node2vec_model.pkl"))
...
plt.tight_layout()
plt.savefig(figure_file("compare_confusion_matrices.png"), dpi=300)
plt.show()
