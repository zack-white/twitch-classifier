...
import os
import json
from project_paths import input_file, model_file, artifact_data_file, config_file
os.environ['KMP_DUPLICATE_LIB_OK']='True'

# === Load Node Features ===
try:
    df = pd.read_csv(input_file('large_twitch_features.csv'))
...
try:
    ...
    edges_df = pd.read_csv(input_file('large_twitch_edges.csv'))
    edges_df = edges_df[
...
try:
    ...
    joblib.dump(clf, model_file('xgboost_node2vec_model.pkl'))
    
    # Save metadata
    np.save(artifact_data_file('node2vec_embeddings.npy'), embeddings)
    np.save(artifact_data_file('y_labels.npy'), y)
    ...
    with open(config_file('xgboost_node2vec_meta.json'), 'w') as f:
        json.dump(meta, f, indent=4)

    # Save the LabelEncoder
    joblib.dump(le, model_file('label_encoder.pkl'))
